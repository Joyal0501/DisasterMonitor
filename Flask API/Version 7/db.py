...
def get():
    ...
    with conn.cursor() as cursor:
        result = cursor.execute('SELECT * FROM Main Where Count > 3;')
...
def update(Tweet, Lat, Long, City):
    ...
    with conn.cursor() as cursor:
        ...
        conn.commit()
    with conn.cursor() as cursor:
        AddMain = "INSERT IGNORE INTO Main (City, Count) VALUES(%s, %s)"
        cursor.execute(AddMain, (City, int(0)))
        conn.commit()
    with conn.cursor() as cursor:
        ...
        conn.commit()
    with conn.cursor() as cursor:
        MainTable= "Update Main SET Count= Count +1 WHERE city = '" +City+"'"
...
def data(City):
    conn = open_connection()
    with conn.cursor() as cursor:
        query = "select ANY_VALUE(Tweet) as Tweet, ANY_VALUE(Latitude) as Latitude, ANY_VALUE(Longitude) as Longitude, ANY_VALUE(Date) as Date , Count(*) as Count from " + City + " Group BY Round(SQRT((Round(Latitude/0.001)*Round(Latitude/0.001))+(Round(Longitude/0.001)*Round(Longitude/0.001)))/10), DATE(Date) Having count(*) > 3;"
